...
def main() -> None:
    ...
    invoice = oblodai.payments.create(
        amount="25", currency="USDT", network="tron", order_id=f"demo-{uuid.uuid4()}"
    )
    print(f"1. invoice {invoice.uuid} for {invoice.payer_amount} {invoice.payer_currency}")

    oblodai.sandbox.simulate_deposit(
        invoice_id=invoice.uuid, amount="25", confirmations=20, txid=f"demo-tx-{uuid.uuid4()}"
    )
    paid = oblodai.payments.get_info(uuid=invoice.uuid)
    print(f"2. simulated deposit -> status {paid.status} (paid={is_payment_paid(paid.status)})")

    oblodai.sandbox.faucet(asset="USDT", amount="100")
    balance = oblodai.account.get_balance()
    ...
    for delivery in oblodai.sandbox.list_webhooks().first():
