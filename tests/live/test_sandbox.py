...
from oblodai import Oblodai, PaymentView, is_payment_paid, verify_webhook
...
@pytest.fixture(scope="module")
def invoice(client: Oblodai) -> PaymentView:
...
def test_reads_public_catalog_data_without_credentials(live_url: str) -> None:
    with Oblodai(base_url=live_url, allow_insecure_base_url=True) as public:
        assert len(public.checkout.list_currencies().currencies) > 0


def test_creates_an_invoice_and_reads_it_back(client: Oblodai, invoice: PaymentView) -> None:
    assert invoice.status == "created"
    assert client.payments.get_info(order_id=invoice.order_id).uuid == invoice.uuid

    listing = client.payments.list_history(limit=5).first()
    ...
    hooks = client.sandbox.list_webhooks().first()
...
def test_deposit_then_payout(client: Oblodai, invoice: PaymentView) -> None:
    client.sandbox.simulate_deposit(
    ...
    assert payout.uuid
    assert client.payouts.get_info(uuid=payout.uuid).order_id == payout.order_id
...
def test_verifies_a_webhook_the_core_signs(client: Oblodai) -> None:
    ...
    endpoint = client.webhooks.register(url=hook_url)
    result = client.webhooks.send_test_payment(
