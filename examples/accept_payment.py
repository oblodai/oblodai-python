...
def main() -> None:
    ...
    invoice = oblodai.payments.create(
        amount="25",  # decimal string (or Decimal), never a float
    ...
    while time.time() < deadline:
        current = oblodai.payments.get_info(uuid=invoice.uuid)
        ...
        if is_payment_final(current.status):
            if is_payment_paid(current.status):
                ...
                resolved = oblodai.payments.resolve(uuid=invoice.uuid, action="accept")
