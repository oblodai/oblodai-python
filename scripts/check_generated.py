"""Fail when what the generator owns is not what it makes of the gateway's contract.

The generator owns ``src/oblodai/generated``, the method tables of ``README.md`` / ``README.ru.md``
(between the ``sdkgen:methods`` markers) and the names in ``names.lock``. This regenerates into a
temporary directory with the backend's ``tools/sdkgen`` (from ``services/core/api/openapi.json``,
with ``-frozen-lock``: a lock that lacks a new name fails instead of being extended) and compares
file by file.
...
def main(argv: List[str]) -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for readme in READMES:
            shutil.copy(ROOT / readme, Path(tmp) / readme)
        cmd = [
            "go",
            "run",
            "./cmd/sdkgen",
        ...
        bad += [r for r in READMES if not filecmp.cmp(Path(tmp) / r, ROOT / r, shallow=False)]
        if bad:
            print(
                "check_generated: generated code or README method tables are stale ("
                + ", ".join(bad)
